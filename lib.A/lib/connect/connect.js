 ...
 */
import {makeId} from '../js/utils/utils.js';
import {database} from '../../../firebase.js';
import {logFactory} from '../../lib/js/utils/log.js';

const log = logFactory(logFactory.flags.comms, 'Comms', 'orange');
...
export const Actor = class {
  ...
  listen() {
    rendezvous.child(this.id).on('child_added', async(snap => {
      ...
      snap.ref.remove();
      log(`Actor[${this.id}]: ring from [${peer}] on channel [${remoteSessionId}]`);
      ...
      if (ch) {
        ...
        log(`Actor[${this.id}]: closing channel to [${ch.peer}]`);
        ch.close();
      }
  ...
  open(remoteSessionId) {
    log(`Actor[${this.id}]: opening a channel to [${remoteSessionId}] from [${this.sessionId}]`);
...
export const Channel = class {
  ...
  open() {
    ...
    const listener = this.theirs.on('child_added', async(snap => {
      ...
      log(`received from [${this.peer}]:`, msg);
      snap.ref.remove();
      this.onreceive(msg);
    }));
